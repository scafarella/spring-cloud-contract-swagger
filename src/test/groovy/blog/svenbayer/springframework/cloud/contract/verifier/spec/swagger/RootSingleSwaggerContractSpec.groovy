 ...
 * @author Sven Bayer
 */
class RootSingleSwaggerContractSpec extends Specification {
    ...
    def "should convert from single swagger to contract"() {
        given:
        File singleSwaggerYaml = new File(SwaggerContractConverterSpec.getResource("/swagger/root_single_swagger.yml").toURI())
        Contract expectedContract = Contract.make {
            ...
            request {
                method(POST())
                urlPath("/coffee-rocket-service/v1.0/") {
                    queryParameters {
